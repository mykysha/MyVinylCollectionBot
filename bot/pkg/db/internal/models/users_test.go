...
func testUserToManyOwnerWishlists(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.OwnerWishlists().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadOwnerWishlists(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OwnerWishlists); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.OwnerWishlists = nil
	if err = a.L.LoadOwnerWishlists(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.OwnerWishlists); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManySetOpLocations(t *testing.T) {
	var err error
	...
	err = a.SetLocations(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetLocations(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.Locations[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.Locations[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpLocations(t *testing.T) {
	var err error
	...
	err = a.AddLocations(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveLocations(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.Locations) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.Locations[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.Locations[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}

func testUserToManyAddOpOwnerWishlists(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddOwnerWishlists(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.OwnerWishlists[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.OwnerWishlists[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.OwnerWishlists().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManySetOpOwnerWishlists(t *testing.T) {
	var err error
	...
	err = a.SetOwnerWishlists(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetOwnerWishlists(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.OwnerWishlists[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.OwnerWishlists[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpOwnerWishlists(t *testing.T) {
	var err error
	...
	err = a.AddOwnerWishlists(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveOwnerWishlists(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.OwnerWishlists) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.OwnerWishlists[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.OwnerWishlists[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Telegramid`: `integer`, `Username`: `character varying`}
	_           = bytes.MinRead
)
