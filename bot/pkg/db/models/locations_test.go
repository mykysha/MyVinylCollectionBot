...
func testLocationToManyCollections(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadCollections(ctx, tx, false, (*[]*Location)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testLocationToManyAddOpCollections(t *testing.T) {
	var err error
...
func testLocationToManySetOpCollections(t *testing.T) {
	var err error
...
func testLocationToManyRemoveOpCollections(t *testing.T) {
	var err error
...
func testLocationToOneUserUsingUser(t *testing.T) {
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, locationDBTypes, true, locationColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Location struct: %s", err)
	}
...
func testLocationToOneRemoveOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	locationDBTypes = map[string]string{`ID`: `integer`, `UserID`: `integer`, `Name`: `character varying`}
	_               = bytes.MinRead
)
